...
class OuterFrame{
    constructor() {
        this.frameName = ['chuan', 'chuan_boss', 'yu', 'yu_boss', 'hua', 'hua_boss', 'qun', 'qun_boss', 'shi'];
        this.frame = [];
        for(let name of this.frameName){
            this.frame[name] = new LazyImage('./resources/bg/' + name + '.png');
        }
    }
}
...
function drawOuterFrame(ctx, power, myLord, outerFrame, logicalWidth, logicalHeight){
    if(outerFrame){
        let img = undefined;
        if(power === "chuan" && myLord){
            img =  outerFrame.frame['chuan_boss'].get();
        }else if(power === "chuan" && !myLord){
            img =  outerFrame.frame['chuan'].get();
        }else if(power === "yu" && myLord){
            img =  outerFrame.frame['yu_boss'].get();
        }else if(power === "yu" && !myLord){
            img =  outerFrame.frame['yu'].get();
        }else if(power === "hua" && myLord){
            img =  outerFrame.frame['hua_boss'].get();
        }else if(power === "hua" && !myLord){
            img =  outerFrame.frame['hua'].get();
        }else if(power === "qun" && myLord){
            img =  outerFrame.frame['qun_boss'].get();
        }else if(power === "qun" && !myLord){
            img =  outerFrame.frame['qun'].get();
        }else if(power === "shi"){
            img =  outerFrame.frame['shi'].get();
        }else{
...
function drawTitleAndName(ctx, title, name, skillTop){
    ...
    let x = power === "shi" ? 355 - offset / 2 : 61 - offset / 2;
    ...
    x = power === "shi" ? 355 - offset / 2 : 60 - offset / 2;
...
function refreshAll(){
    ...
    // 刷新是否为主公
    myLord = power === 'shi' ? false : myLordButton.checked;
...
draw();
importIllustration("./resources/example-lzh.png");
importMiscellaneous();
